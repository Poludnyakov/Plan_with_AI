...
from dashboard_router import get_cookie_secret, get_current_user_tg_id, sign_tg_id
from database import get_db
from models import User
from telegram_miniapp_auth import validate_telegram_init_data


logger = logging.getLogger("MiniAppUIRouter")
...
async def mini_calendar(
    request: Request,
    user_tg_id: int = Depends(get_current_user_tg_id),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(select(User).filter(User.tg_id == user_tg_id))
    user = result.scalar_one_or_none()
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Пользователь не найден")
    return templates.TemplateResponse(
        request=request,
        name="mini_calendar.html",
...
async def miniapp_auth(
    ...
    logger.info("Telegram Mini App UI session created for tg_id=%s", tg_id)
    return response
